...
// グループ①: ヒアリング・試作（同時期） グループ②: 見積
export const CYCLE_STAGES = ["hearing", "prototype", "estimate"] as const;
...
export function getCycleNextOptions(currentStage: string): string[] {
  ...
  // グループ①(ヒアリング・試作) → 同グループ + 見積へ
  if (group === 1) {
    return ["hearing", "prototype", "estimate", "decided"].filter((s) => s !== currentStage);
  }
  // グループ②(見積) → グループ①に戻る + 確定へ
  if (group === 2) {
    return ["hearing", "prototype", "estimate", "decided"].filter((s) => s !== currentStage);
  }
  return ["decided"];
